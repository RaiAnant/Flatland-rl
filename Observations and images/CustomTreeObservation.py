...
class CustomTreeObservation(TreeObsForRailEnv):
    ...
    def add_to_safe_map(self, pos, dir):
        if pos == (9, 15):
            print('here')
    ...
    def get(self, handle: int = 0) -> Node:
        ...
        root_node_observation = Node(dist_own_target_encountered=0, dist_other_target_encountered=0,
                                     ...
                                     childs={}, start = None, end = None, dir = None, safe_zones=None)
    ...
    def get_many(self, handles: Optional[List[int]] = None) -> Dict[int, Node]:
        ...
        for h in observations.keys():
            if (observations[h] is not None and min(observations[h].childs['F'][4],
                    ...
                    (h not in self.obs_dict or observations[h].childs['F'][13] != self.obs_dict[h][-1][h].childs['F'][
                        13]):
                if h not in self.obs_dict:
                    self.obs_dict[h] = []
                self.obs_dict[h].append(observations.copy())
